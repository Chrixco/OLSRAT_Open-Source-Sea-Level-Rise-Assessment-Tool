...
class SlrVmNewGui(QDialog):
    def __init__(self, provider, parent=None):
        ...
        self.setWindowTitle("SLR Vulnerability Mapper - Professional Toolkit")
        self.resize(1300, 750)

        QgsMessageLog.logMessage("Initialising SLR Vulnerability Toolkit GUI", "SLR Mapper", Qgis.Info)
        ...
        title_label = QLabel("<h1 style='color: white; margin: 0; font-size: 32px; font-weight: bold; letter-spacing: 1px;'>SLR Vulnerability Mapper</h1>")
        title_label.setStyleSheet("background: transparent;")
        text_layout.addWidget(title_label)

        subtitle_label = QLabel("<p style='color: rgba(255, 255, 255, 0.98); margin: 0; font-size: 15px; font-weight: 600; letter-spacing: 0.5px;'>Professional Climate Adaptation & Coastal Resilience Toolkit</p>")
    ...
    def open_contact(self):
        ...
        QMessageBox.information(
            ...
            "<p><b>GitHub:</b> <a href='https://github.com/christianxcorral'>@christianxcorral</a></p>"
            "<br>"
            "<p>For bug reports, feature requests, or collaboration inquiries, please visit:</p>"
            "<p><a href='https://github.com/christianxcorral/slr_vulnerability_mapper/issues'>GitHub Issues</a></p>"
        )
    ...
    def open_donate(self):
        ...
        QMessageBox.information(
            ...
            "<ul>"
            "<li>⭐ Starring the project on <a href='https://github.com/christianxcorral/slr_vulnerability_mapper'>GitHub</a></li>"
    ...
    def create_category_button(self, category):
        QgsMessageLog.logMessage(f"Creating category button for {category['name']}", "SLR Mapper", Qgis.Info)
        ...
        for alg in category["algorithms"]:
            ...
            if alg.get("special"):
                if alg["name"] == "separator":
                    ...
                    QgsMessageLog.logMessage(f"Adding special interactive viz action", "SLR Mapper", Qgis.Info)
                    action = menu.addAction(alg["name"])
                    action.triggered.connect(self.open_interactive_visualization)
                    continue

            QgsMessageLog.logMessage(f"Adding menu action for {alg['name']} with ID {alg['id']}", "SLR Mapper", Qgis.Info)
            action = menu.addAction(alg["name"])
            action.triggered.connect(partial(self._run_algorithm_with_debug, alg["id"], alg["name"]))

        def show_menu():
            QgsMessageLog.logMessage(f"Showing menu for {category['name']}", "SLR Mapper", Qgis.Info)
            menu.setMinimumWidth(btn.width())
            # Use built-in Qt positioning for better cross-platform support
            menu.exec_(btn.mapToGlobal(btn.rect().bottomLeft()))
            QgsMessageLog.logMessage(f"Menu closed for {category['name']}", "SLR Mapper", Qgis.Info)
    ...
    def open_interactive_visualization(self):
        ...
        QgsMessageLog.logMessage("Opening Interactive Data Visualization Dashboard", "SLR Mapper", Qgis.Info)
        try:
            ...
            QgsMessageLog.logMessage(
                ...
                "SLR Mapper", level=Qgis.Critical
            )
            from qgis.PyQt.QtWidgets import QMessageBox
    ...
    def _run_algorithm_with_debug(self, alg_id, alg_name):
        QgsMessageLog.logMessage(f"Opening Processing dialog for {alg_name} (ID={alg_id})", "SLR Mapper", Qgis.Info)
        try:
            ...
            if result:
                QgsMessageLog.logMessage(f"Algorithm '{alg_name}' completed successfully", "SLR Mapper", Qgis.Info)
            else:
                QgsMessageLog.logMessage(f"Algorithm dialog closed without running (user canceled)", "SLR Mapper", Qgis.Info)
